...
export const ApiService = {
  // Funciones para Usuarios Asistidos
  getUser: async (userId: string) => {
    try {
      const document = await firestore().collection('Usuarios').doc(userId).get()
      if (document.exists) {
  ...
  getCategory: async (categoryId: string) => {
    try {
      const document = await firestore().collection('Categorías').doc(categoryId).get()
      if (document.exists) {
        return { id: document.id, ...document.data() }
      } else {
  ...
  // Funciones para Grupos de Apoyo
  getSupportGroups: async (userId: string) => {
    try {
      const snapshot = await firestore().collection('supportGroups').where('members', 'array-contains', userId).get()
      return snapshot.docs.map((doc: FirebaseFirestoreTypes.QueryDocumentSnapshot) => ({
        id: doc.id,
        ...doc.data(),
      }))
  ...
  // Funciones para Pictogramas
  getPictograms: async (groupId: string) => {
    try {
      const snapshot = await firestore().collection('pictograms').where('groupId', '==', groupId).get()
      return snapshot.docs.map((doc: FirebaseFirestoreTypes.QueryDocumentSnapshot) => ({
        id: doc.id,
        ...doc.data(),
      }))
